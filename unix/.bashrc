...
export IP2=$(ip link show eth1 >/dev/null 2>&1 && ip -4 addr show eth1 | grep -oP '(?<=inet\s)\d+(\.\d+){3}' || echo "")
...
alias dlcip='docker inspect -f "{{range .NetworkSettings.Networks}}{{println .IPAddress}}{{end}}" $(docker ps -l -q) | paste -sd " "'  # Get IP's of last Docker container
