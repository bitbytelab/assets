...
unset HISTFILE
HISTSIZE=100 HISTFILESIZE=1000 HISTCONTROL=ignoreboth shopt -s histappend checkwinsize
...
# uv generate-shell-completion bash > /usr/share/bash-completion/completions/uv  		## or ## echo 'eval "$(uv generate-shell-completion bash)"' >> ~/.bashrc
# uvx --generate-shell-completion bash > /usr/share/bash-completion/completions/uvx  	## or ## echo 'eval "$(uvx --generate-shell-completion bash)"' >> ~/.bashrc
# sudo docker completion bash | sudo tee /usr/share/bash-completion/completions/docker > /dev/null
...
# load .bashrc from cloud # In-memory execution: Throttled to once every 24 hours (86400s)
_UP=$(stat -c %Y ~/.bashrc_sync 2>/dev/null || echo 0) _URL="https://raw.githubusercontent.com/bitbytelab/assets/main/unix/.bashrc"
# (( $(date +%s) - _UP > 86400 )) && _SRC=$(curl -sSf --max-time 3 $_URL 2>/dev/null) && touch ~/.bashrc_sync && . <(echo "${_SRC:-$(cat ~/.bashrc_fallback 2>/dev/null)}") && [ -n "$_SRC" ] && echo "$_SRC" > ~/.bashrc_fallback
